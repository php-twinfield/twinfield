...
class DimensionTypeApiConnector extends BaseApiConnector
{
    ...
    public function listAll(
        ...
        foreach ($response->data->Items->ArrayOfString as $dimensionTypeArray) {
            $dimensionType = new DimensionType();

            if (isset($dimensionTypeArray->string[0])) {
                $dimensionType->setCode($dimensionTypeArray->string[0]);
                $dimensionType->setName($dimensionTypeArray->string[1]);
            } else {
                $dimensionType->setCode($dimensionTypeArray[0]);
                $dimensionType->setName($dimensionTypeArray[1]);
            }
